...
public class MainActivity extends Activity {
    ...
    private WebView webView; private ValueCallback<Uri[]> filePathCallback;
    ...
    @Override protected void onCreate(Bundle savedInstanceState){super.onCreate(savedInstanceState);webView=new WebView(this);setContentView(webView);WebSettings s=webView.getSettings();s.setJavaScriptEnabled(true);s.setDomStorageEnabled(true);s.setDatabaseEnabled(true);s.setAllowFileAccess(true);s.setAllowContentAccess(true);s.setLoadWithOverviewMode(false);s.setUseWideViewPort(false);s.setMediaPlaybackRequiresUserGesture(true);s.setCacheMode(WebSettings.LOAD_NO_CACHE);webView.addJavascriptInterface(new AndroidBridge(),"MojiAndroid");CookieManager.getInstance().setAcceptCookie(true);CookieManager.getInstance().setAcceptThirdPartyCookies(webView,false);
      ...
      webView.setDownloadListener(new DownloadListener(){@Override public void onDownloadStart(String url,String ua,String cd,String mt,long len){if(url==null||(!url.startsWith("https://")&&!url.startsWith("http://"))){Toast.makeText(MainActivity.this,"ไม่สามารถดาวน์โหลดไฟล์รูปแบบนี้",Toast.LENGTH_LONG).show();return;}try{DownloadManager.Request r=new DownloadManager.Request(Uri.parse(url));r.setMimeType(mt);r.addRequestHeader("User-Agent",ua);r.setNotificationVisibility(DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED);r.setDestinationInExternalPublicDir(Environment.DIRECTORY_DOWNLOADS,"moji-backup");((DownloadManager)getSystemService(Context.DOWNLOAD_SERVICE)).enqueue(r);}catch(Exception e){Toast.makeText(MainActivity.this,"บันทึกไฟล์ไม่สำเร็จ",Toast.LENGTH_SHORT).show();}}});
      if(savedInstanceState==null)webView.loadUrl(APP_URL+"?android=v66");else webView.restoreState(savedInstanceState);
    }

    public class AndroidBridge{
      @JavascriptInterface public void saveBackup(String json){runOnUiThread(()->{try{String stamp=new SimpleDateFormat("yyyyMMdd-HHmmss",Locale.US).format(new Date());String name="moji-backup-"+stamp+".json";ContentValues v=new ContentValues();v.put(MediaStore.Downloads.DISPLAY_NAME,name);v.put(MediaStore.Downloads.MIME_TYPE,"application/json");v.put(MediaStore.Downloads.RELATIVE_PATH,Environment.DIRECTORY_DOWNLOADS+"/Moji");Uri uri=getContentResolver().insert(MediaStore.Downloads.EXTERNAL_CONTENT_URI,v);if(uri==null)throw new Exception("create failed");try(OutputStream out=getContentResolver().openOutputStream(uri)){if(out==null)throw new Exception("open failed");out.write(json.getBytes(StandardCharsets.UTF_8));}Toast.makeText(MainActivity.this,"สำรองข้อมูลแล้ว: Downloads/Moji/"+name,Toast.LENGTH_LONG).show();}catch(Exception e){Toast.makeText(MainActivity.this,"สำรองข้อมูลไม่สำเร็จ",Toast.LENGTH_LONG).show();}});}
    }

    private void injectEnhancements(WebView view){StringBuilder js=new StringBuilder();js.append("(function(){const files=[");for(int i=0;i<ENHANCEMENT_SCRIPTS.length;i++){if(i>0)js.append(",");js.append("'").append(ENHANCEMENT_SCRIPTS[i]).append("'");}js.append("];let p=Promise.resolve();files.forEach(function(f){p=p.then(function(){return new Promise(function(resolve){const base=f.split('?')[0];document.querySelectorAll('script[data-moji-android]').forEach(function(s){if((s.src||'').indexOf(base)>=0)s.remove();});const s=document.createElement('script');s.dataset.mojiAndroid='1';s.src='./'+f+'&android='+(Date.now());s.onload=resolve;s.onerror=resolve;document.body.appendChild(s);});});});p.then(function(){if(window.MojiAndroid){window.backup=function(){try{MojiAndroid.saveBackup(JSON.stringify(S));}catch(e){alert('สำรองข้อมูลไม่สำเร็จ');}};}const b=[...document.querySelectorAll('header button')].find(x=>(x.getAttribute('onclick')||'').includes('settings(true)'));if(b){b.innerHTML='<svg viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"#342d25\" stroke-width=\"2.4\" stroke-linecap=\"round\" stroke-linejoin=\"round\" style=\"width:28px;height:28px;display:block\"><circle cx=\"12\" cy=\"12\" r=\"3\"/><circle cx=\"12\" cy=\"12\" r=\"7\"/><path d=\"M12 2v3M12 19v3M2 12h3M19 12h3M4.93 4.93l2.12 2.12M16.95 16.95l2.12 2.12M4.93 19.07l2.12-2.12M16.95 7.05l2.12-2.12\"/></svg>';b.setAttribute('aria-label','ตั้งค่า');b.setAttribute('title','ตั้งค่า');b.style.display='inline-flex';b.style.alignItems='center';b.style.justifyContent='center';}});})();");view.evaluateJavascript(js.toString(),null);}
