// M Personal Finance — clean baseline financial goals + cross-page sync.
// Single source of truth: S.goals. No schema/key changes.
(function(){
  ...
  function syncWhatIf(){
    const card=document.getElementById('whatIfCard');if(!card)return;
  ...
  function apply(){hideDuplicateFutureSummary();mountFutureGoals();syncWhatIf();syncDashboard()}
  ...
  const obs=new MutationObserver(()=>setTimeout(apply,0));obs.observe(document.documentElement,{childList:true,subtree:true});
  apply();
})();
